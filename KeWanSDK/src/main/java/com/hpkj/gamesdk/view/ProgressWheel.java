...
public class ProgressWheel extends View {
    ...
    int matProg_progressIndeterminate;
    int matProg_barColor;
    int matProg_rimColor;
    int matProg_rimWidth;
    int matProg_spinSpeed;
    int matProg_barSpinCycleTime;
    int matProg_circleRadius;
    int matProg_fillRadius;
    int matProg_barWidth;
    int matProg_linearProgress;
    /**
     * The constructor for the ProgressWheel
     */
    public ProgressWheel(Context context, AttributeSet attrs) {
        super(context, attrs);
        int[] kewan_ProgressWheel= ResourceUtil.getStyleableIntArray(context,"kewan_ProgressWheel");
        matProg_progressIndeterminate=ResourceUtil.getStyleableFieldId(context,"kewan_ProgressWheel","matProg_progressIndeterminate");
        matProg_barColor=ResourceUtil.getStyleableFieldId(context,"kewan_ProgressWheel","matProg_barColor");
        matProg_rimColor=ResourceUtil.getStyleableFieldId(context,"kewan_ProgressWheel","matProg_rimColor");
        matProg_rimWidth=ResourceUtil.getStyleableFieldId(context,"kewan_ProgressWheel","matProg_rimWidth");
        matProg_spinSpeed=ResourceUtil.getStyleableFieldId(context,"kewan_ProgressWheel","matProg_spinSpeed");
        matProg_barSpinCycleTime=ResourceUtil.getStyleableFieldId(context,"kewan_ProgressWheel","matProg_barSpinCycleTime");
        matProg_circleRadius=ResourceUtil.getStyleableFieldId(context,"kewan_ProgressWheel","matProg_circleRadius");
        matProg_fillRadius=ResourceUtil.getStyleableFieldId(context,"kewan_ProgressWheel","matProg_fillRadius");
        matProg_barWidth=ResourceUtil.getStyleableFieldId(context,"kewan_ProgressWheel","matProg_barWidth");
        matProg_linearProgress=ResourceUtil.getStyleableFieldId(context,"kewan_ProgressWheel","matProg_linearProgress");
        parseAttributes(context.obtainStyledAttributes(attrs, kewan_ProgressWheel));
    ...
    private void parseAttributes(TypedArray a) {
        ...
        circleRadius =
                (int) a.getDimension(matProg_circleRadius, circleRadius);

        fillRadius = a.getBoolean(matProg_fillRadius, false);

        barWidth = (int) a.getDimension(matProg_barWidth, barWidth);

        rimWidth = (int) a.getDimension(matProg_rimWidth, rimWidth);

        float baseSpinSpeed =
                a.getFloat(matProg_spinSpeed, spinSpeed / 360.0f);
        spinSpeed = baseSpinSpeed * 360;

        barSpinCycleTime =
                a.getInt(matProg_barSpinCycleTime, (int) barSpinCycleTime);

        barColor = a.getColor(matProg_barColor, barColor);

        rimColor = a.getColor(matProg_rimColor, rimColor);

        linearProgress = a.getBoolean(matProg_linearProgress, false);

        if (a.getBoolean(matProg_progressIndeterminate, false)) {
            spin();
        }
